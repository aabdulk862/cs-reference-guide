...
class HealthStatus(Enum):
    HEALTHY = "healthy"
    UNHEALTHY = "unhealthy"
    DRAINING = "draining"  # No new requests, finish in-flight
...
class ServerHealth:
    ...
    error_rate_window: deque = field(default_factory=lambda: deque(maxlen=100))
...
class HealthChecker:
    ...
    def __init__(self, servers: list[str], config: Optional[HealthCheckConfig] = None):
        self.config = config or HealthCheckConfig()
        self.servers: dict[str, ServerHealth] = {
            addr: ServerHealth(address=addr) for addr in servers
        }
        self._running = False
    ...
    async def _check_server(self, health: ServerHealth):
        """Perform active health check against a server."""
        url = f"http://{health.address}{self.config.path}"
        start = time.perf_counter()
        
        try:
            async with self._session.get(url) as response:
                elapsed = (time.perf_counter() - start) * 1000
                health.last_response_time_ms = elapsed
                health.last_check_time = time.time()
                
                if response.status == self.config.expected_status:
                    self._record_success(health)
                else:
                    self._record_failure(health, f"Status {response.status}")
    ...
    def record_request_result(self, address: str, success: bool, 
                              response_time_ms: float):
        """Passive health monitoring — called after each real request."""
        health = self.servers.get(address)
        if not health:
            return
        
        health.error_rate_window.append(0 if success else 1)
        health.last_response_time_ms = response_time_ms
        
        # Circuit breaker: if error rate exceeds threshold, mark unhealthy
        if health.error_rate > 0.5 and len(health.error_rate_window) >= 10:
            if health.status == HealthStatus.HEALTHY:
                print(f"Circuit breaker tripped for {address} "
                      f"(error rate: {health.error_rate:.1%})")
                health.status = HealthStatus.UNHEALTHY
                health.consecutive_failures = self.config.unhealthy_threshold
    
    def _record_success(self, health: ServerHealth):
        health.consecutive_successes += 1
        health.consecutive_failures = 0
        health.error_rate_window.append(0)
        
        if (health.status == HealthStatus.UNHEALTHY and 
            health.consecutive_successes >= self.config.healthy_threshold):
            print(f"Server {health.address} recovered → HEALTHY")
            health.status = HealthStatus.HEALTHY
    
    def _record_failure(self, health: ServerHealth, reason: str):
        health.consecutive_failures += 1
        health.consecutive_successes = 0
        health.error_rate_window.append(1)
        
        if (health.status == HealthStatus.HEALTHY and 
            health.consecutive_failures >= self.config.unhealthy_threshold):
            print(f"Server {health.address} failed → UNHEALTHY ({reason})")
            health.status = HealthStatus.UNHEALTHY
    
    def get_healthy_servers(self) -> list[str]:
        return [
            h.address for h in self.servers.values()
            if h.status == HealthStatus.HEALTHY
        ]
    
    def drain_server(self, address: str):
        """Mark server for draining (graceful removal)."""
        if address in self.servers:
            self.servers[address].status = HealthStatus.DRAINING
            print(f"Server {address} marked for draining")
```
