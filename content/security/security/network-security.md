...
class NetworkIDS:
    ...
    def __init__(self):
        ...
        self.CONNECTION_RATE_THRESHOLD = 100 # connections per minute
        self.LATERAL_MOVEMENT_THRESHOLD = 10 # unique internal destinations
        self.DATA_EXFIL_THRESHOLD = 100_000_000  # 100MB outbound

    def process_connection(self, event: ConnectionEvent) -> List[Alert]:
        """Process a network connection event and check for anomalies."""
        alerts = []
        metrics = self.metrics[event.source_ip]

        metrics.total_connections += 1
        metrics.unique_destinations.add(event.dest_ip)
        metrics.ports_scanned.add(event.dest_port)
        metrics.bytes_transferred += event.bytes_sent
        ...
        window = metrics.last_seen - metrics.first_seen
        if window < 60 and len(metrics.ports_scanned) > self.PORT_SCAN_THRESHOLD:
            alerts.append(Alert(
                severity='HIGH',
                type='PORT_SCAN',
                source=event.source_ip,
                message=f'Port scan detected: {len(metrics.ports_scanned)} ports in {window:.0f}s',
            ))

        # Detection: Lateral movement (many internal destinations)
        internal_dests = [ip for ip in metrics.unique_destinations if self.is_internal(ip)]
        if len(internal_dests) > self.LATERAL_MOVEMENT_THRESHOLD:
            alerts.append(Alert(
                severity='CRITICAL',
                type='LATERAL_MOVEMENT',
                source=event.source_ip,
                message=f'Possible lateral movement: {len(internal_dests)} internal hosts contacted',
            ))

        # Detection: Data exfiltration (large outbound transfer)
        if (not self.is_internal(event.dest_ip) and
                metrics.bytes_transferred > self.DATA_EXFIL_THRESHOLD):
            alerts.append(Alert(
                severity='CRITICAL',
                type='DATA_EXFILTRATION',
                source=event.source_ip,
                message=f'Large outbound transfer: {metrics.bytes_transferred / 1e6:.1f}MB',
            ))

        # Detection: Brute force (high failure rate)
        if metrics.failed_connections > 10 and metrics.failed_connections / metrics.total_connections > 0.8:
            alerts.append(Alert(
                severity='HIGH',
                type='BRUTE_FORCE',
                source=event.source_ip,
                message=f'High failure rate: {metrics.failed_connections}/{metrics.total_connections}',
            ))
